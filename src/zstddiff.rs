...
fn calc_chunk_num(s1: &mut impl Seek, s2: &mut impl Seek, l1: Option<u64>, l2: Option<u64>) -> Result<(f64, u64, u64, f64, f64)> {
...
pub fn diff(old: &mut (impl Read+Seek), new: &mut (impl Read+Seek), dest: &mut (impl Write+Seek), level: Option<i32>, old_len_hint: Option<u64>, new_len_hint: Option<u64>) -> Result<()> {
    ...
    let (num_chunks, old_len, new_len, olf, nlf) = calc_chunk_num(old, new, old_len_hint, new_len_hint)?;
...
pub fn apply(old: &mut (impl Read+Seek), diff: &mut (impl Read+Seek), dest: &mut impl Write, old_len_hint: Option<u64>) -> Result<u64> {
